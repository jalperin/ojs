...
/**
 * @file plugins/importexport/crossrefbb/classes/CrossRefBBExportDom.inc.php
 *
 * Copyright (c) 2003-2013 John Willinsky
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class CrossRefBBExportDom
 * @ingroup plugins_importexport_crossrefbb_classes
 *
...
if (!class_exists('DOIExportDom')) { // Bug #7848
	import('plugins.importexport.crossrefbb.classes.DOIExportDom');
}

// XML attributes
define('CROSSREFBB_XMLNS' , 'http://www.crossref.org/schema/4.3.0');
define('CROSSREFBB_XSI_SCHEMAVERSION' , '4.3.0');
define('CROSSREFBB_XSI_SCHEMALOCATION' , 'http://www.crossref.org/schema/4.3.0 http://www.crossref.org/schema/deposit/crossref4.3.0.xsd');

class CrossRefBBExportDom extends DOIExportDom {
	 ...
	 */
	function CrossRefBBExportDom(&$request, &$plugin, &$journal, &$objectCache) {
		// Configure the DOM.
	...
	function getXmlSchemaVersion() {
		return CROSSREFBB_XSI_SCHEMAVERSION;
	}
	...
	function getXmlSchemaLocation() {
		return CROSSREFBB_XSI_SCHEMALOCATION;
	}
	...
	function &_generateJournalArticleDom(&$doc, &$journal, &$issue, &$section, &$article) {
		...
		// DOI data node
		$DOIdataNode =& $this->_generateDOIdataDom($doc, $article->getPubId('doi'), Request::url(null, 'article', 'view', $article->getBestArticleId()));
	 ...
	 * @param $url string
	 */
	function &_generateDOIdataDom(&$doc, $DOI, $url) {
