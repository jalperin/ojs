...
class AlmExportPlugin extends ImportExportPlugin {
	...
	function getDisplayName() {
		return Locale::translate('plugins.importexport.alm.displayName');
	}

	function getDescription() {
		return Locale::translate('plugins.importexport.alm.description');
	}
	...
	function executeCLI($scriptName, &$args) {
		...
		if (!$journal) {
			if ($journalPath != '') {
				echo Locale::translate('plugins.importexport.alm.cliError') . "\n";
				echo Locale::translate('plugins.importexport.alm.error.unknownJournal', array('journalPath' => $journalPath)) . "\n\n";
			}
			...
			if (!$doiPrefix) {
				echo Locale::translate('plugins.importexport.crossref.errors.noDOIprefix') . "\n";
				return;
			}
		}
		...
		if ($outFile != '') switch (array_shift($args)) {
            case 'all':
                ...
                if (!$this->exportArticles($articles, $outFile)) {
                    echo Locale::translate('plugins.importexport.alm.cliError') . "\n";
                    echo Locale::translate('plugins.importexport.alm.export.error.couldNotWrite', array('fileName' => $outFile)) . "\n\n";
                }
			...
			case 'issue':
				...
				if ($issue == null) {
					echo Locale::translate('plugins.importexport.alm.cliError') . "\n";
					echo Locale::translate('plugins.importexport.alm.export.error.issueNotFound', array('issueId' => $issueId)) . "\n\n";
					return;
				}
				$issues = array($issue);
	...
	function usage($scriptName) {
		echo Locale::translate('plugins.importexport.alm.cliUsage', array(
