...
<script>
	...
	{if $timedViewsJson}
		timed_views = {$timedViewsJson};
		data_all[0].sources.push(timed_views)
	{/if}
	{literal}
...
var categories = [{ldelim} name: "html", display_name: "HTML Views", tooltip_text: "{translate key="plugins.generic.alm.categories.html"}" },
		...
		data[0]["sources"].forEach(function(source) {
			...
			if (total > 0) {
				if (!categoryRow) {
					...
					var categoryTitle = categoryRow.append("h5")
						.attr("class", "alm-category-row-heading")
						.attr("id", "month-" + category.name)
						.text(category.display_name);

					var tooltip = categoryTitle.append("div")
